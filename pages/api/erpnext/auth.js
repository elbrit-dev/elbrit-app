  ...
  try {
    ...
    if (phoneNumber && !email) {
      ...
      for (const phoneToSearch of phoneVariations) {
        ...
        const employeeSearchParams = new URLSearchParams({
          ...
          fields: JSON.stringify(['name', 'first_name', 'cell_number', 'fsl_whatsapp_number', 'company_email', 'role_id', 'kly_role_id', 'status'])
        });
      ...
      if (employeeResponse.ok) {
        ...
        if (employeeResult.data) {
          ...
          userData = {
            ...
            roleName: 'Admin',
            kly_role_id: employee.role_id || employee.kly_role_id || null, // Add role ID field
    ...
    // Otherwise, search by company_email (for Microsoft SSO)
      ...
      const employeeSearchParams = new URLSearchParams({
        ...
        fields: JSON.stringify(['name', 'first_name', 'employee_name', 'cell_number', 'fsl_whatsapp_number', 'company_email', 'role_id', 'kly_role_id', 'status', 'department', 'designation', 'date_of_joining', 'date_of_birth'])
      });
    ...
    let teamsData = null;
    try {
      if (userData.role_id || userData.kly_role_id) {
        // Use GraphQL to fetch ElbritRoleIDS for specific role
        console.log('🔍 Fetching teams and warehouses via GraphQL for role ID:', userData.role_id || userData.kly_role_id);
        ...
        const graphqlResponse = await fetch(graphqlUrl, {
          ...
          body: JSON.stringify({
            query: graphqlQuery,
            variables: {
              name: userData.role_id || userData.kly_role_id
            }
          })
        });
        ...
        if (graphqlResponse.ok) {
          ...
          if (roleData) {
            ...
            console.warn('⚠️ No role data found for role ID:', userData.role_id || userData.kly_role_id);
          }
        } else {
